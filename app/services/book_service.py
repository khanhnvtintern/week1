...
def _get_book_with_relations(db: Session, book_id: int) -> Book:
    book = db.execute(
        select(Book).options(joinedload(Book.author), joinedload(Book.category)).where(Book.id==book_id)
    ).scalar_one_or_none()
    if book is None:
...
def list_books(db : Session) -> list[BookRead]:
    books = db.scalars(
        select(Book)
        .options(joinedload(Book.author), joinedload(Book.category)).order_by(Book.id)).all()
    return [to_book_read(book) for book in books]


def get_book(db: Session, book_id: int) -> BookRead:
    return to_book_read(_get_book_with_relations(db, book_id))
